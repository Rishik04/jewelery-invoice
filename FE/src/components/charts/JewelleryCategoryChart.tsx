...
const TABS = [
  { key: "GOLD_22K", label: "Gold 22K", cat: "GOLD", karat: "22K", barFrom: "#8b5cf6", barTo: "#a855f7", gradId: "gTab22K", active: "bg-indigo-50 text-indigo-700 border-indigo-200", inactive: "text-gray-500 hover:text-indigo-600", pill: "bg-indigo-50", pillText: "text-indigo-700", pillBadge: "bg-indigo-100", dot: "#6366f1" },
  { key: "GOLD_18K", label: "Gold 18K", cat: "GOLD", karat: "18K", barFrom: "#8b5cf6", barTo: "#a855f7", gradId: "gTab18K", active: "bg-purple-50 text-purple-700 border-purple-200", inactive: "text-gray-500 hover:text-purple-600", pill: "bg-purple-50", pillText: "text-purple-700", pillBadge: "bg-purple-100", dot: "#8b5cf6" },
  { key: "SILVER",   label: "Silver",   cat: "SILVER", karat: null, barFrom: "#06b6d4", barTo: "#6366f1", gradId: "gTabSilver", active: "bg-cyan-50 text-cyan-700 border-cyan-200", inactive: "text-gray-500 hover:text-cyan-600", pill: "bg-cyan-50", pillText: "text-cyan-700", pillBadge: "bg-cyan-100", dot: "#06b6d4" },
] as const;
...
interface JewelleryCategoryChartProps {
  invoices: any[];
  isLoading?: boolean;
}

const JewelleryCategoryChart = ({ invoices, isLoading }: JewelleryCategoryChartProps) => {
    ...
    for (const tab of TABS) {
      ...
      for (const inv of invoices) {
        for (const item of inv.items ?? []) {
          ...
          if (cat !== tab.cat || (tab.karat !== null && karat !== tab.karat)) continue;
      ...
      result[tab.key] = {
        ...
        totalWeight: Number(allItems.reduce((s, i) => s + i.weight, 0).toFixed(2)),
        totalRevenueK: Math.round(allItems.reduce((s, i) => s + i.revenueK, 0) * 10) / 10,
        totalCount: allItems.reduce((s, i) => s + i.count, 0),
      ...
      {currentData && currentData.totalCount > 0 && (
        <div className="flex gap-3 mb-5">
          <div className={`flex-1 ${currentTab.pill} rounded-2xl px-3 py-3`}>
            <p className={`text-xs font-bold uppercase tracking-widest ${currentTab.pillText} mb-1`}>Total weight</p>
          ...
          <div className={`flex-1 ${currentTab.pill} rounded-2xl px-3 py-3`}>
            <p className={`text-xs font-bold uppercase tracking-widest ${currentTab.pillText} mb-1`}>Revenue</p>
            <p className={`text-lg font-bold ${currentTab.pillText}`}>₹{currentData.totalRevenueK}K</p>
            <p className="text-[10px] text-gray-400 mt-0.5">{currentData.totalItems} Items</p>
          </div>
          <div className={`flex-1 ${currentTab.pill} rounded-2xl px-3 py-3`}>
            <p className={`text-xs font-bold uppercase tracking-widest ${currentTab.pillText} mb-1`}>Items sold</p>
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis type="number" tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} unit="g" />
              ...
              <Bar dataKey="weight" radius={[0, 8, 8, 0]} maxBarSize={28}>
                {currentData.items.map((_: any, i: number) => (
