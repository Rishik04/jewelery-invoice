...
export const InputField = React.memo(
  React.forwardRef<HTMLInputElement, any>(
    ({ icon: Icon, type = "text", value, onChange, error, label, disabled, ...props }, ref) => (
      <motion.div className="relative group">
        ...
        <input
          ...
          disabled={disabled}
          className={`w-full px-4 py-3.5 border-2 rounded-xl outline-none
...
const DescriptionInput = ({ value, onChange, typ, onProductSelect, error, ref }: any) => {
...
const EMPTY_ITEM = { type: "G", id: "", description: "", hsnCode: "7113", purity: "", grossWeight: 0, netWeight: 0, rate: 0, makingCharges: 0, otherCharges: 0, makingChargesType: "PERCENT" };
...
const ModernInvoiceForm = () => {
  ...
  const validateStep = () => {
    ...
    if (step === 2) {
      formData.items.forEach((item, index) => {
        if (!item.id?.trim()) e[`items.${index}.description`] = "Please select the valid product"
    ...
    setErrors(e); if (Object.keys(e).length > 0) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      await printInvoiceMutation.mutateAsync(payload);
    } catch (err) {
      console.error(err);
    }
  };
              ...
              {step === 1 && (
                <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                    <InputField label="Customer Name" icon={User} placeholder="Ramesh Sharma" value={formData.customer.name} error={errors["customer.name"]} onChange={(v: string) => handleInputChange("customer", v, null, "name")} ref={setRef("customer.name")} />
                    <InputField label="Phone" icon={Phone} placeholder="9876543210" value={formData.customer.phone} error={errors["customer.phone"]} onChange={(v: string) => handleInputChange("customer", v, null, "phone")} ref={setRef("customer.phone")} />
                    <InputField label="Address" icon={MapPin} placeholder="123 MG Road, Mumbai" value={formData.customer.address} error={errors["customer.address"]} onChange={(v: string) => handleInputChange("customer", v, null, "address")} ref={setRef("customer.address")} />
                    <InputField label="Invoice Date" icon={Calendar} type="date" value={formData.date} error={errors["customer.date"]} onChange={(v: string) => handleInputChange("date", v)} ref={setRef("customer.date")} />
                  </div>
                      ...
                      <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                          <SelectField label="Metal" icon={Coins} value={item.type} onChange={(v: string) => handleInputChange("type", v, index)}
                            options={[{ value: "G", label: "Gold" }, { value: "S", label: "Silver" }]} placeholder="Select metal" />
                          <DescriptionInput
                            value={item.description}
                            typ={item.type}
                            onChange={(v: string) => handleInputChange("description", v, index)}
                            error={errors[`items.${index}.description`]}
                            ref={setRef(`items.${index}.description`)}
                          ...
                          <InputField label="Purity" icon={Sparkles} value={item.purity} onChange={(v: string) => handleInputChange("purity", v, index)} disabled={true} placeholder={"Auto-filled"}/>
                          <InputField label="Gross Weight (g)" icon={Calculator} type="number" value={item.grossWeight} error={errors[`items.${index}.netWeight`]} onChange={(v: string) => handleInputChange("grossWeight", v, index)} ref={setRef(`items.${index}.netWeight`)} />
                          <InputField label="Net Weight (g)" icon={Calculator} type="number" value={item.netWeight} error={errors[`items.${index}.netWeight`]} onChange={(v: string) => handleInputChange("netWeight", v, index)} ref={setRef(`items.${index}.netWeight`)} />
                          <InputField label="Other Charges (₹)" icon={Calculator} type="number" value={item.otherCharges} error={errors[`items.${index}.otherCharges`]} onChange={(v: string) => handleInputChange("otherCharges", v, index)} ref={setRef(`items.${index}.otherCharges`)} />
                          <InputField label="Rate / 10g (₹)" icon={Calculator} type="number" value={item.rate} error={errors[`items.${index}.rate`]} onChange={(v: string) => handleInputChange("rate", v, index)} ref={setRef(`items.${index}.rate`)} />
